...
import { Accessibility, Dog, ExternalLink, MapPin, Navigation, Users, Volleyball } from 'lucide-react';
...
export function AboutTab({ beach, waterQuality = null, weather = null }: AboutTabProps) {
  ...
  const hasFamilyAmenities =
    amenities && (amenities.restrooms || amenities.lifeguard !== 'none' || amenities.wheelchairAccessible);
  const hasDogAmenities = amenities && amenities.dogFriendly;
          ...
          <ul className="space-y-2">
            {personality.differentiators.map((item) => (
              <li key={item} className="flex items-start gap-2 text-sand-700">
                <span className="mt-1 w-2 h-2 rounded-full bg-ocean-400 shrink-0" aria-hidden="true" />
                {item}
              </li>
            ))}
          </ul>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {amenities.lifeguard !== 'none' && (
                  <AmenityChip
                    ...
                    label={amenities.lifeguard === 'year-round' ? 'Year-round lifeguard' : 'Seasonal lifeguard'}
                  />
                )}
                {amenities.wheelchairAccessible && (
                  <AmenityChip icon={<Accessibility className="w-4 h-4" />} label="Wheelchair accessible" />
                )}
              </div>
            </div>
          )}
