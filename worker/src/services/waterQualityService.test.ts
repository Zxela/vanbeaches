...
describe('waterQualityService', () => {
  ...
  it('writes water quality data to KV with 21600s TTL', async () => {
    ...
    expect(mockKv.put).toHaveBeenCalledWith('waterquality:english-bay', expect.any(String), {
      expirationTtl: 21600,
    });
  });
  ...
  it('matches the WaterQualityStatus shared type shape', async () => {
    ...
    expect(result.ecoliCount === null || typeof result.ecoliCount === 'number').toBe(true);
    expect(result.advisoryReason === null || typeof result.advisoryReason === 'string').toBe(true);
    expect(result.sampleDate === null || typeof result.sampleDate === 'string').toBe(true);
